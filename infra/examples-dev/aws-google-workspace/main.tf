...
# Redis backend to share access tokens across multiple lambda instances
resource "aws_elasticache_cluster" "redis-backend" {
  count                = length(module.worklytics_connector_specs.enabled_oauth_long_access_connectors) > 0 ? 1 : 0
  cluster_id           = "psoxy-redis-backend"
  engine               = "redis"
